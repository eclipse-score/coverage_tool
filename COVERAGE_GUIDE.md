...
coverage pipeline in `@score_coverage` works: the mechanism, the
...
3. **Installs the per-test tool**
   (`--coverage_output_generator=@score_coverage//:merger`). After
...
**Lives in `@score_coverage` (shared):**
...
- **Runfiles paths span repositories.** The reporter wrapper mixes files
  from `_main` (the consumer), `score_coverage` and toolchain repos, so every
...
- **The baseline manifest lists consumer files.** The reporter resolves
  ...
  "current repository" would resolve against `score_coverage` and find
  nothing.
